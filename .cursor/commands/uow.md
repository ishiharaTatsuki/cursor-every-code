...
- 生成物:
  - `records/<project>/image/codex_request/codex_request_<TASK_ID>.md`（例: `records/trademaster/image/codex_request/codex_request_WP1.6.md`）
  - `records/<project>/handover/handover_<YYYY-MM-DD>.md`（引き継ぎの種）
- **重要**: スクリプト実行後、`Next:` に表示される **完全な request パス** をそのまま `--request` に使う（手作業で埋める必要なし）
...
**手順**: 2) で `ops.py uow` 実行後、`Next:` に出る request パス（例: `records/trademaster/image/codex_request/codex_request_WP1.6.md`）をそのまま `--request` に渡す。

```bash
# 例（ops.py uow の出力に出るパスをそのまま使う）
python .cursor/scripts/codex_loop.py \
  --request records/trademaster/image/codex_request/codex_request_WP1.6.md \
  --min-coverage 80 --max-quality-cycles 25 --max-blocked-repeats 5
```

- 1回だけ実行する場合: `codex exec - < <ops.py uow の Next に出る request パス>`
