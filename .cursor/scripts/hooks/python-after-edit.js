...
function main() {
    ...
    // Skip ruff for nested repos / third-party vendored code (avoids recurring git diffs)
    const ruffExcludePrefixes = (process.env.ECC_RUFF_EXCLUDE_PREFIXES || "dydxbot/v4-clients").split(/[,;\s]+/);
    if (ruffExcludePrefixes.some((p) => p && (rel.startsWith(p + "/") || rel === p || rel.startsWith(p + "\\")))) {
        process.exit(0);
    }
