 ...
 * Apply Skill Pack
 *
 * Moves skill directories between:
 *   - .cursor/skills (enabled)
 *   - .cursor/skills-optional (disabled/optional)
 *
 * This is intentionally a simple filesystem move (no symlinks) for cross-platform use.
 */
...
Usage:
  node .cursor/scripts/apply-skill-pack.js --pack <pack-name> [--dry-run]
  node .cursor/scripts/apply-skill-pack.js --list

Notes:
  - Packs are defined under .cursor/skill-packs/*.json
  - The script moves directories between:
      .cursor/skills
...
function main() {
  ...
  if (list) {
    if (!fs.existsSync(packsDir)) {
      console.log("No skill-packs directory found.");
      process.exit(0);
    }
  ...
  // Detect duplicates (same skill exists in both enabled and optional)
  const dup = [...enabled].filter((s) => optional.has(s));
  if (dup.length > 0) {
    console.error("Error: duplicate skill directories detected in both skills and skills-optional:");
    for (const s of dup) console.error("  - " + s);
    console.error("Please remove duplicates before applying a pack.");
    process.exit(1);
  }
