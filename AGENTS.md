...
## Working agreements
- Follow a TDD loop (RED → GREEN → REFACTOR) when adding or changing behavior.
- Always ensure tests pass and keep changes minimal.
- Treat input validation and path handling as security-sensitive; do not weaken existing validation.
- Do not add new dependencies unless explicitly requested.
- Prefer using the repo's canonical tooling commands (see .cursor/scripts/recommend-commands.js).
...
### Command execution stability (freeze prevention)

- Prefer short, single-purpose commands; avoid large multi-line inline scripts in approval prompts.
- Prefer running script files or compact one-liners over heredoc/here-string (`@"..."@`) commands.
- Add explicit timeouts to long-running commands and retry with smaller command chunks if UI stalls.
- Minimize approval prompts by reusing approved command prefixes when possible.
- If an execution is interrupted, check for and clean up leftover `pwsh`/`python` processes before retry.
