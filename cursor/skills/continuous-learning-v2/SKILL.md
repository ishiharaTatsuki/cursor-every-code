...
| Feature | v1 | v2 |
|---------|----|----|
| Observation | SessionEnd hook (transcript-based, metadata + /learn prompt) | PreToolUse/PostToolUse (observation log) |
...
└─────────────────────────────────────────┘
      │
      │ Observer agent reads (background, Composer 1)
      ▼
┌─────────────────────────────────────────┐
│          PATTERN DETECTION              │
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "$CLAUDE_PROJECT_DIR/.cursor/skills/continuous-learning-v2/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": "$CLAUDE_PROJECT_DIR/.cursor/skills/continuous-learning-v2/hooks/observe.sh post"
      }]
    }]
  }
}
...
- Stop hook can still be used for lightweight checks (independent of v2)
