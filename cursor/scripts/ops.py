...
def cmd_uow(ctx: RepoContext, args: argparse.Namespace) -> int:
    ...
    reqs_dir = str(ctx.config.get("codex_requests_dirname", "image/codex_request"))
    print("\nNext:")
    print("  1) 生成された codex_request_*.md の TODO を埋める")
    print(
        "  2) 自動ループ（推奨）: python .cursor/scripts/codex_loop.py "
        f"--request records/<project>/{reqs_dir}/codex_request_<TASK>.md --min-coverage 80"
    )
    print("     - 1回だけ: codex exec - < records/.../codex_request_XXX.md")
    return 0
