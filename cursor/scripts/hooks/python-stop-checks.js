...
function main() {
    ...
    const ruffExcludePrefixes = (process.env.ECC_RUFF_EXCLUDE_PREFIXES || "dydxbot/v4-clients").split(/[,;\s]+/);
    const filteredRel = changedRel.filter((p) => !ruffExcludePrefixes.some((pref) => pref && (p.startsWith(pref + "/") || p === pref || p.startsWith(pref + "\\"))));
    if (filteredRel.length === 0) process.exit(0);

    const runner = detectRunner(projectDir);
    const changedAbs = filteredRel.map((p) => (path.isAbsolute(p) ? p : path.join(projectDir, p)));

    // 1) ruff (project-wideは重い時もあるので、まず changed files に限定)
    if ((process.env.ECC_SKIP_RUFF || "").trim() !== "1") {
        // If ruff isn't available in this environment, skip silently.
        if (probeTool(projectDir, runner, "ruff", "ruff")) {
